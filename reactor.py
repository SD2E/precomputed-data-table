...
def launch_omics(m, r):
    ...
    app_job_def_inputs = {}
    app_job_def_inputs['input_data'] = agaveutils.to_agave_uri(systemId="data-sd2e-community",
                                                               dirPath=input_counts_path)
...
def launch_app(m, r):
    ...
    r.logger.info(
        "experiment_ref: {} data_converge_dir: {} analysis: {}".format(experiment_ref, data_converge_dir, analysis))
    (storage_system, dirpath, leafdir) = agaveutils.from_agave_uri(data_converge_dir)
    root_dir = StorageSystem(storage_system, agave=r.client).root_dir
    data_converge_dir2 = join_posix_agave([root_dir, dirpath, leafdir])
    r.logger.info("data_converge_dir2: {}".format(data_converge_dir2))
    ...
    if analysis == "perform-metrics":
        ...
        job_def, product_patterns = ea_pm.get_job_template("data-sd2e-projects.sd2e-project-48", output_path_parent,
                                                           data_converge_dir, experiment_ref)
    else:
        ...
        # maxRunTime should probably be determined based on experiment size
        parameter_dict = {"experiment_ref": experiment_ref, "data_converge_dir": data_converge_dir2,
                          "analysis": analysis}
        if control_dir:
