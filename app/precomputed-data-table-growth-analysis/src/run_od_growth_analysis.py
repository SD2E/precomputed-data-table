...
def run_od_analysis(platereader_df, exp_ref, fcs_meta_df: Optional[DataFrame] = None):
...
# def run_od_analysis(exp_ref, exp_ref_dir, data_confirm_dict):
#
#     pr_df, pr_fname = grab_pr_dataframe(exp_ref, exp_ref_dir)
#
#     if data_confirm_dict['fc_raw_log10']:
#         fcs_meta_df, fcs_meta_fname = grab_meta_dataframe(exp_ref, exp_ref_dir)
#         pr_analysis_df, fcs_analysis_df = growth_analysis(pr_df, exp_ref, fcs_meta_df)
#
#         return pr_analysis_df, pr_fname, fcs_analysis_df, fcs_meta_fname
#
#     else:
#         pr_analysis_df = growth_analysis(pr_df, exp_ref)
#
#         return pr_analysis_df, pr_fname
...
def main(exp_ref, analysis, out_dir, data_converge_dir):
    ...
    if data_confirm_dict['fc_raw_log10']:
        ...
        pr_analysis_df, fcs_analysis_df = run_od_analysis(pr_df, exp_ref, fcs_meta_df)
        fcs_growth_fname = 'pdt_{}__fcs_growth_analysis.csv'.format(exp_ref)
    else:
        pr_analysis_df = run_od_analysis(pr_df, exp_ref)
